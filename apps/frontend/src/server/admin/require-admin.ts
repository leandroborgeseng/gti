...
export function jsonAdminError(status: number, message: string): NextResponse {
...
export async function requireAdmin(
  req: Request,
  forbiddenMessage = "Apenas administradores podem realizar esta operação."
  ...
  if (!token) {
    return { ok: false, response: jsonAdminError(401, "Não autenticado.") };
  }
  ...
  try {
    ...
    return { ok: false, response: jsonAdminError(401, "Sessão inválida.") };
  }

  if (session.mustChangePassword) {
    return {
      ok: false,
      response: jsonAdminError(403, "Altere a sua senha antes de continuar.")
    };
  }
  ...
  if (session.role !== "ADMIN") {
    return {
      ok: false,
      response: jsonAdminError(403, forbiddenMessage)
    };
  }
