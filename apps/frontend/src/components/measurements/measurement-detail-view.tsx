...
import { getMeasurement, type Measurement } from "@/lib/api";
...
export function MeasurementDetailView({
  ...
  const q = useQuery({
    queryKey: queryKeys.measurement(initial.id),
    queryFn: () => getMeasurement(initial.id),
    initialData: initial
  });
  const measurement = q.data ?? initial;

  function applyMeasurement(next: Measurement): void {
    qc.setQueryData(queryKeys.measurement(measurement.id), next);
  ...
  const usedServiceIds = measurement.items?.filter((i) => i.type === "SERVICE").map((i) => i.referenceId) ?? [];
  const showAddLines = measurement.status === "OPEN" && (tipo === "DATACENTER" || tipo === "INFRA");
  ...
  const orgLabel = measurement.contract?.organization
    ...
    : "-";
  const supplierLabel = measurement.contract?.supplier?.name ?? measurement.contract?.companyName ?? "-";
    ...
    <div className="space-y-4">
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Status:{" "}
              <span className="font-medium text-slate-900">{statusLabel[measurement.status] ?? measurement.status}</span>
            </p>
          </div>
          <MeasurementActions
            measurementId={measurement.id}
            measurementStatus={measurement.status}
            onUpdated={applyMeasurement}
          />
        </div>
        <div className="mt-4 grid gap-2 text-sm text-slate-700 md:grid-cols-2">
          <p>
            <strong className="text-slate-900">Código interno:</strong> {measurement.contract?.internalCode ?? "-"}
          </p>
          <p>
        ...
        <p className="mt-4 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs leading-relaxed text-slate-600">
          <strong className="text-slate-800">Composição:</strong> as linhas são geradas automaticamente a partir dos itens
          contratuais vigentes na competência (com proporção por dias quando a vigência é parcial). Informe quantidades
          em itens sob demanda ou únicos e use «Calcular medição».
        </p>
      </Card>
      ...
      <Card className="p-5">
        ...
        {!(measurement.items && measurement.items.length > 0) ? (
          <p className="text-sm text-slate-500">
            Nenhum item na medição. Contratos sem itens vigentes na competência precisam ter a precificação cadastrada
            antes de criar a medição.
          </p>
        ) : null}
      ...
      <Card className="p-5">
        ...
        <p className="mb-3 text-sm text-slate-600">
          Glosas automáticas vêm do cálculo (funcionalidades não validadas). Glosas manuais exigem justificativa e entram
          no líquido da medição.
        </p>
      ...
      <Card className="sticky bottom-2 border-slate-300 bg-white/95 p-5 shadow-sm backdrop-blur">
        ...
        <div className="grid gap-1 text-sm text-slate-700 sm:grid-cols-2">
          ...
          <p>
            Glosas automáticas: <span className="tabular-nums font-medium text-slate-900">{formatBrl(autoGlosas)}</span>
          </p>
          <p>
            Glosas manuais: <span className="tabular-nums font-medium text-slate-900">{formatBrl(manualGlosas)}</span>
          </p>
          <p>
            Valor glosado (total):{" "}
            <span className="tabular-nums font-medium text-slate-900">{formatBrl(measurement.totalGlosedValue)}</span>
          </p>
          <p className="sm:col-span-2">
            Líquido (≥ 0): <span className="tabular-nums text-base font-semibold text-slate-900">{formatBrl(net)}</span>
          </p>
        </div>
      </Card>
    </div>
