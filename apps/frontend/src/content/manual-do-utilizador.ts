...
export const MANUAL_LAST_UPDATED = "2026-08-21"; // Revisão ticket 96: histórico/anulação de entrega
    ...
    blocks: [
      {
        ...
        parts: [
          ...
          " para ler comunicados formais, dar ciência e enviar manifestações quando exigido. A lista pagina no servidor (10, 25, 50 ou 100 por página). Se houver itens vinculados à notificação, a empresa também indica status e justificativa por item. Análises internas feitas pela fiscalização não são exibidas para a empresa."
        ]
      },
      {
        kind: "p",
        parts: [
          "Na ficha do contrato (usuários internos), a seção Notificações permite criar a partir de um modelo, revisar, assinar com senha do SIGTI e enviar por e-mail. Em Administração → Modelos de notificação, cadastre os textos-base com campos de mala direta. A Central de Documentos (`/documentos` para internos e `/externo/documentos` para a empresa) lista notificações formalizadas com paginação no servidor (10, 25, 50 ou 100 por página) e filtros rápidos (pendentes de minha assinatura, assinados por mim, elaborados por mim). A lista interna segue o órgão do contexto ativo. Ao mudar o filtro, a listagem volta à primeira página."
        ]
      },
      {
        kind: "tip",
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Aditivos, reajustes e histórico: na ficha do contrato, registre termos aditivos, reajustes, repactuações e demais alterações com tipo, referência do instrumento, data de formalização, início dos efeitos, novo término (opcional) e descrição/observação obrigatória. Selecione os itens contratuais afetados para informar novos valores, percentual de reajuste/acréscimo/supressão ou incluir um item novo; a supressão (ou «Encerrar item») encerra a vigência do item (não o apaga). Em «Acrescentar quantidade», informe só a quantidade a somar à vigente. Em «Renovar quantidade/período», informe a nova quantidade e o que fazer com o saldo remanescente (expira, acumula ou continua). Antes de confirmar, confira o resumo comparativo (antes/depois, diferença em R$ e %). Os valores passam a valer a partir da data de início dos efeitos e o valor global vigente é recalculado pela composição dos itens (o valor global original é preservado). A mesma seção mostra o histórico automático (contratação original + cada aditivo), com detalhe expansível dos itens; não há edição/exclusão comum — apenas cancelamento formal com justificativa. Com medições ou aditivos, a edição direta dos itens de precificação fica bloqueada: alterações financeiras devem seguir por aditivo.",
          ...
          "Histórico auditável dos itens: na aba Auditoria do contrato, consulte quem inseriu, excluiu ou alterou módulos, funcionalidades, grupos de validação e serviços (antes/depois de Código do Item, descrição, criticidade, estado e entrega). A anulação de um evento de entrega também entra nesta lista, com a justificativa. A lista é paginada no servidor (10, 25, 50 ou 100 por página), com filtros por período, autor, tipo de item, ação e busca textual."
        ]
      },
      {
        kind: "p",
    ...
    blocks: [
      {
        ...
        parts: [
          { href: "/measurements", label: "Medições" },
          " · Uma medição por contrato e por competência (mês/ano). Cada registro percorre estados: Aberta → Em revisão ou Glosada → Aprovada. O valor aprovado consolida o que conta como medição aprovada para relatórios. A listagem pagina no servidor (10, 25, 50 ou 100 por página); a pesquisa e a troca de tamanho de página voltam à primeira página."
        ]
      },
      {
        kind: "ul",
        items: [
          ...
          "Após «Calcular medição», a seção «Memória de saldo e entrega» mostra a data de corte, o saldo por item (contratado → já medido → saldo → consumo não medido → medição atual) e o snapshot das funcionalidades vigentes nessa data (não o estado atual). Com a medição aprovada, essa memória permanece como registro histórico.",
    ...
    blocks: [
      {
        ...
        parts: [
          ...
          " · Consulta consolidada das glosas geradas nas medições (automáticas e manuais), paginada no servidor (10, 25, 50 ou 100 por página). Novas glosas adicionais são registradas na própria tela da medição, na seção «Glosas e descontos».",
