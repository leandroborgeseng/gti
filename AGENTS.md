...
- **Railway / migrações órfãs:** `scripts/prisma-entry-preflight.cjs` corre antes do `migrate deploy`. Se `_prisma_migrations` tiver nomes que já não existem em `prisma/migrations`, o schema `public` é **reiniciado automaticamente** (sem variável). Para não apagar nesse caso: `PRISMA_NO_AUTO_WIPE_ON_LEGACY_DRIFT=1`. Reinício forçado em todo o arranque: `PRISMA_FRESH_PUBLIC_SCHEMA_ON_BOOT=1`. Baseline: `20260419100000_baseline_full_schema`.
